...
def make_grayscale(input_image):
    ...
    decode_array_image=cv2.imdecode(image_array,cv2.IMREAD_UNCHANGED)
    print("decode values of image:",decode_array_image)


    converted_gray_image=cv2.cvtColor(decode_array_image,cv2.COLOR_RGB2GRAY)
